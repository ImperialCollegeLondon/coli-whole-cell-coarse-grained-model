...
{
    // do the stochastic reactions
    prepareForSteps (cellState) ;
    while ( t < duration )
    {
        doStep ( cellState , duration ) ;
        if (cellState->get_prot_Level() >= mf_ModelParameters->P_div_threshold)
        {
            if (mf_ModelParameters->linear_growth) cellState->L += cellState->alpha * t;
            else cellState->L *= exp(cellState->alpha * t);
            return t;
        }
    }
    if (mf_ModelParameters->linear_growth) cellState->L += cellState->alpha * t;
    else cellState->L *= exp ( cellState->alpha * duration );
    return duration;
}
