...
function x = scrXX_fit_coreg_low_delta_model_to_scott_and_dai_data_3_pars()
...
sigma_start = 6.47; % because 22 aa / s and 7336 aas per ribosomes (Dai et al) / 1.67 (extended ribosome)
K_start = 0.11 * 0.76; % because 0.11 from Dai elong rate vs R/P Km and 0.76 because conversion from R/P to fR (extended rib Scott)
fQ_start = 0.5;

%%%% 0.5524    6.0935    0.1121 from 0.5 / 6.47 / 0.0836

x = fminsearch(@(x)(simulate_scott_dai_and_compute_cost_coreg_low_delta(x)), [fQ_start, sigma_start, K_start]);

end


function composition_data = simulate_scott_dai_coreg_low_delta(x)
pars.sigma = x(2); 
pars.fQ = x(1);
pars.K = x(3);
...
function [cost,cost_fR,cost_useless,cost_elong,cost_active_rib_frac] = simulate_scott_dai_and_compute_cost_coreg_low_delta(x)
x
comp_data = simulate_scott_dai_coreg_low_delta(x);
