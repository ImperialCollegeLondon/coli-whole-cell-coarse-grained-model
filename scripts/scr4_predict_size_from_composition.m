...
for i_m=1:length(model_names)
    ...
    for sector=single_sectors
        ...
        for data_str={'data_nut_cm','data_nut_useless','data_nut_only','data'}
            for fX_str={'true','false'}
                ...
                % predictions
                out_dir = ['../results-data/res4_basan-2015-si-2017-taheri-2015-fit/' model_names{i_m} '/' sector{1} '_' data_str{1} '_fX-' fX_str{1}];
                mkdir(out_dir);
        ...
        for i_sector_2=i_sector_1+1:length(single_sectors)
            ...
            for data_str={'data_nut_cm','data_nut_useless','data_nut_only','data'}
                for fX_str={'true','false'}
                    ...
                    % make and write tables with the regression parameters and the
                    % predictions
                    out_dir = ['../results-data/res4_basan-2015-si-2017-taheri-2015-fit/' model_names{i_m} '/' sector_1{1} '_and_' sector_2{1} '_' data_str{1} '_fX-' fX_str{1}];
                    mkdir(out_dir);
                    writetable(table(scale_factor),[out_dir '/scale_factor.csv']);
                    writetable(table(exponents),[out_dir '/exponents.csv']);
                    eval(['real = ' data_str{1} '.cell_size;']);
            ...
            for i_sector_3=i_sector_2+1:length(single_sectors)
                ...
                for data_str={'data'}
                    for fX_str={'true','false'}
                        ...
                        % make and write tables with the regression parameters and the
                        % predictions
                        out_dir = ['../results-data/res4_basan-2015-si-2017-taheri-2015-fit/' model_names{i_m} '/' sector_1{1} '_and_' sector_2{1} '_and_' sector_3{1} '_' data_str{1} '_fX-' fX_str{1}];
                        mkdir(out_dir);
                        writetable(table(scale_factor),[out_dir '/scale_factor.csv']);
                        writetable(table(exponents),[out_dir '/exponents.csv']);
                        eval(['real = ' data_str{1} '.cell_size;']);
    ...
    table_creation_str = [table_str table_row_str];
    eval(table_creation_str);
    writetable(results,['../results-data/res4_basan-2015-si-2017-taheri-2015-fit/three-sectors-size-predictions-R2-values_' model_names{i_m} '.csv'],'WriteRowNames',true);
